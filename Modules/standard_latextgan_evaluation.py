...
def tweet_generator(generator, autoencoder, word2vec_model: gensim.models.word2vec.Word2Vec, num_tweets: int=1):
  ...
  for _ in range(num_tweets):
    ...
    print(f"{' '.join([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states=generator(noise), training=False) if i.numpy()[0] != 0])}")
    print()
...
def bleu4_score(generator, autoencoder, word2vec_model: gensim.models.word2vec.Word2Vec, reference_data: [[int]], num_tweets: int=1):
  ...
  for _ in range(num_tweets):
    noise = tf.random.normal([1, 100])
    generated_tweet.append([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states=generator(noise), training=False) if i.numpy()[0] != 0])
