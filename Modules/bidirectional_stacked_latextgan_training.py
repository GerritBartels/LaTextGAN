...
def gradient_penalty(discriminator, real_tweet, generated_tweet):
  ...
  alpha_1 = tf.random.uniform(shape=[real_tweet[0].shape[0], 1], minval=0, maxval=1)
  alpha_2 = tf.random.uniform(shape=[real_tweet[1].shape[0], 1], minval=0, maxval=1)

  interpolate_1 = alpha_1*real_tweet[0] + (1-alpha_1)*generated_tweet[0]
  interpolate_2 = alpha_2*real_tweet[1] + (1-alpha_2)*generated_tweet[0]

  output = discriminator(interpolate_1, interpolate_2)

  gradients_1 = tf.gradients(output, interpolate_1)
  gradients_2 = tf.gradients(output, interpolate_2)

  gradient_norm_1 = tf.sqrt(tf.reduce_sum(tf.square(gradients_1)))
  gradient_norm_2 = tf.sqrt(tf.reduce_sum(tf.square(gradients_2)))

  penalty_1 = 10*tf.reduce_mean((gradient_norm_1-1.)**2)
  penalty_2 = 10*tf.reduce_mean((gradient_norm_2-1.)**2)
...
def visualize_GAN(autoencoder, word2vec_model, fixed_input_1, fixed_input_2, random_input_1, random_input_2, train_losses_generator, train_losses_discriminator, num_epochs):
  """Visualize performance of the Generator by feeding predefined random noise vectors through it.
  ...
  Arguments:
    ...
    fixed_input_1 (tensor): List containing predefined random vectors 
    fixed_input_2 (tensor): List containing predefined random vectors 
    random_input_1 (tensor): List containing predefined random vectors
    random_input_2 (tensor): List containing predefined random vectors 
  ...
  print()
  print(f"From Fixed Vector: {' '.join([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states_1=fixed_input_1[0], states_2=fixed_input_1[1], training=False) if i.numpy()[0] != 0])}")
  print(f"From Fixed Vector: {' '.join([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states_1=fixed_input_2[0], states_2=fixed_input_2[1], training=False) if i.numpy()[0] != 0])}")
  print()
  print(f"From Random Vector: {' '.join([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states_1=random_input_1[0], states_2=random_input_1[1], training=False) if i.numpy()[0] != 0])}")
  print(f"From Random Vector: {' '.join([word2vec_model.wv.index2word[i.numpy()[0] -1] for i in autoencoder.Decoder.inference_mode(states_1=random_input_2[0], states_2=random_input_2[1], training=False) if i.numpy()[0] != 0])}")
...
def train_step_GAN(generator, discriminator, train_data, optimizer_generator, optimizer_discriminator, train_generator):
  ...
  # 1.
  noise_1 = tf.random.normal([train_data[0].shape[0], 100])
  noise_2 = tf.random.normal([train_data[1].shape[0], 100]) 

  # Two Gradient Tapes, one for the Discriminator and one for the Generator 
  with tf.GradientTape() as generator_tape, tf.GradientTape() as discriminator_tape:
      # 2.
      generated_tweet_1, generated_tweet_2 = generator(noise_1, noise_2)

      # 3.
      real = discriminator(train_data[0], train_data[1])
      fake = discriminator(generated_tweet_1, generated_tweet_2)

      # 4.
      loss_from_generator = generator_loss(fake)
      # Add gradient penalty to enforce lipschitz continuity
      loss_from_discriminator = discriminator_loss(real, fake) + gradient_penalty(discriminator=discriminator, real_tweet=train_data, generated_tweet=(generated_tweet_1, generated_tweet_2))
...
def train_GAN(generator, discriminator, autoencoder, word2vec_model: gensim.models.word2vec.Word2Vec, train_dataset_GAN: tf.data.Dataset, num_epochs: int=150, running_average_factor: float=0.95, learning_rate: float=0.0001):
  ...
  fixed_generator_input_3 = tf.random.normal([1, 100])
  fixed_generator_input_4 = tf.random.normal([1, 100])
  ...
  for epoch in range(num_epochs):
    ...
    random_generator_input_3 = tf.random.normal([1, 100])
    random_generator_input_4 = tf.random.normal([1, 100])
    
    visualize_GAN(autoencoder=autoencoder,
                  word2vec_model=word2vec_model,
                  fixed_input_1=generator(fixed_generator_input_1, fixed_generator_input_2), 
                  fixed_input_2=generator(fixed_generator_input_3, fixed_generator_input_4), 
                  random_input_1=generator(random_generator_input_1, random_generator_input_2), 
                  random_input_2=generator(random_generator_input_3, random_generator_input_4), 
